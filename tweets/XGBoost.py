...
from gensim.models.doc2vec import TaggedDocument #Labelling tweets for doc2vec purpose
...
train_dataset_path = Path("Datasets/train_tweets.csv")
test_dataset_path = Path("Datasets/train_tweets.csv")
...
train_pred = xgb_cl.predict(xtrain_w2v)
train_pred_int = (train_pred >= 0.3).astype(np.int)

# Calculate metrics
train_f1 = f1_score(ytrain, train_pred_int)
train_accuracy = accuracy_score(ytrain, train_pred_int)
train_cm = confusion_matrix(ytrain, train_pred_int)
...
val_pred = xgb_cl.predict(xvalid_w2v)
val_pred_int = (val_pred >= 0.3).astype(np.int)

# Calculate metrics
val_f1 = f1_score(yvalid, val_pred_int)
val_accuracy = accuracy_score(yvalid, val_pred_int)
val_cm = confusion_matrix(yvalid, val_pred_int)
...
test_pred = xgb_cl.predict(test_w2v)
test_pred_int = (test_pred >= 0.3).astype(np.int)

# Calculate metrics
test_f1 = f1_score(test['label'], test_pred_int)
test_accuracy = accuracy_score(test['label'], test_pred_int)
test_cm = confusion_matrix(test['label'], test_pred_int)
...
print(classification_report(ytrain, train_pred_int))

print("\nValidation Classification Report:")
print(classification_report(yvalid, val_pred_int))